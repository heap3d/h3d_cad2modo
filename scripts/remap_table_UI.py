...
def get_tags_list(mode):
    ...
    if mode == h3dc.RM_SCAN:
        ...
        if not lx.eval(
            "query scriptsysservice userValue.isDefined ? {}".format(
                h3dc.USERVAL_NAME_COLORS_STORE
            )
        ):
            lx.eval(
                "user.defNew {} type:string life:temporary".format(
                    h3dc.USERVAL_NAME_COLORS_STORE
                )
            )
        # store colors string
...
def get_materials_list(mode, tags):
    ...
    if mode == h3dc.RM_SCAN:
        ...
        if not lx.eval(
            "query scriptsysservice userValue.isDefined ? {}".format(
                h3dc.USERVAL_NAME_MATERIALS_STORE
            )
        ):
            lx.eval(
                "user.defNew {} type:string life:temporary".format(
...
def clear_table_ui():
    ...
    while True:
        ...
        if lx.eval(
            "query scriptsysservice userValue.isDefined ? {}".format(
                user_value_name_color
            )
        ):
            lx.eval(
                "!user.defDelete {}".format(user_value_name_color)
            )  # delete existing user value
        else:
            break
        if lx.eval(
            "query scriptsysservice userValue.isDefined ? {}".format(
            ...
            lx.eval(
                "!user.defDelete {}".format(user_value_name_material)
            )  # delete existing user value
        else:
            break
        i += 1
...
def build_line(ui_line, map_index, tags, materials, tags_materials_map):
    ...
    lx.eval(
        "user.defNew {} type:integer life:temporary".format(user_value_name_material)
    )
    lx.eval(
...
def scan_init(tags, materials):
    if not lx.eval(
        "query scriptsysservice userValue.isDefined ? {}".format(
            h3dc.USERVAL_NAME_OVERSIZE
        )
    ):
        lx.eval(
            "user.defNew {} type:boolean life:temporary".format(
                h3dc.USERVAL_NAME_OVERSIZE
            )
        )
    ...
    if not lx.eval(
        ...
        lx.eval(
            "user.defNew {} type:integer life:temporary".format(
                h3dc.USERVAL_NAME_MATERIAL_DEFAULT
            )
        )
    ...
    for mesh in modo.Scene().selectedByType(itype=c.MESH_TYPE):
        ...
        if not mesh.parents:
            ...
            for item in mesh.parents:
                h3dd.print_debug(
                    "parent <{}>: is_visible = {}".format(item.name, is_visible(item)),
                    1,
                )
